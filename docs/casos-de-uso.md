...
| **Flujo normal**| 1. El usuario envía `GET /api/animales/:id/arbol-genealogico?generaciones=3`.<br>2. El sistema verifica que el animal existe.<br>3. El sistema construye recursivamente el árbol (padre → abuelos → bisabuelos).<br>4. Responde con `200 OK` y la estructura del árbol. |
...
```json
{
  "success": true,
  "data": {
    "animal": { "id": "...", "nombre": "Rex" },
    "arbol": {
      "nombre": "Rex",
      "padre": {
        "nombre": "Max",
        "padre": { "nombre": "Rocky", "padre": null, "madre": null },
        "madre": { "nombre": "Luna", "padre": null, "madre": null }
      },
      "madre": {
        "nombre": "Bella",
        "padre": { "nombre": "Toby", "padre": null, "madre": null },
        "madre": { "nombre": "Daisy", "padre": null, "madre": null }
      }
    }
  }
}
...
| **Postcondición**| Se devuelve una lista paginada de animales.                    |
| **Flujo normal**| 1. El usuario envía `GET /api/animales?especie=...&raza=...&sexo=...&page=1&limit=10`.<br>2. El sistema aplica los filtros y devuelve resultados paginados.<br>3. Responde con `200 OK`. |
...
| **Flujo normal**| 1. El usuario envía `PATCH /api/animales/:id/padres` con `{ padre: "id1", madre: "id2" }`.<br>2. El sistema valida que los IDs existen y los sexos son correctos.<br>3. Verifica que no se asignen padres que sean descendientes del animal (evitar ciclos).<br>4. Actualiza el animal.<br>5. Responde con `200 OK`. |
...
| **Flujo normal**| 1. El usuario envía `POST /api/auth/register` con email, password, nombre.<br>2. El sistema valida formato email y password ≥ 6 caracteres.<br>3. Hashea password con bcrypt (salt 10).<br>4. Crea usuario con rol `user` por defecto.<br>5. Responde `201 Created` (sin password en la respuesta). |
...
| **Flujo normal**| 1. El usuario envía `POST /api/auth/login` con email y password.<br>2. El sistema verifica credenciales (email existe, password coincide).<br>3. Genera token JWT con `{ id, rol }` y expiry.<br>4. Responde `200 OK` con token y datos del usuario (sin password). |
...
| **Flujo normal**| 1. El usuario envía `GET /api/auth/profile` con token en header `Authorization: Bearer <token>`.<br>2. El sistema decodifica el token, busca al usuario y devuelve sus datos.<br>3. Responde `200 OK`. |
...
| **Flujo normal**| 1. El usuario envía `PUT /api/animales/:id` con los campos a actualizar.<br>2. El sistema valida los datos.<br>3. Verifica que el usuario es admin o el `propietario` del animal.<br>4. Actualiza el documento.<br>5. Responde `200 OK`. |
...
|-----------------|---------------------------------------------------------------|
| **Actor**       | Usuario autenticado (admin o propietario del animal)           |
| **Descripción** | El usuario desactiva un animal sin borrarlo físicamente        |
| **Precondición**| El animal existe y está activo.                                |
| **Postcondición**| El animal queda con `active: false`.                           |
| **Flujo normal**| 1. El usuario envía `DELETE /api/animales/:id`.<br>2. El sistema verifica permisos.<br>3. Usa `findByIdAndUpdate` con `{ active: false }` (NO elimina físicamente).<br>4. Responde `200 OK` con los datos actualizados. |
...
| **Flujo normal**| 1. Admin envía la solicitud al endpoint correspondiente (`/api/especies`, `/api/razas`).<br>2. El sistema verifica el rol admin.<br>3. Ejecuta la operación.<br>4. Responde según corresponda. |
...
| **Precondición**| El animal existe y está activo.                                |
| **Postcondición**| Se devuelve la lista de hijos/hermanos.                        |
| **Flujo normal**| 1. Usuario envía `GET /api/animales/:id/hijos` o `GET /api/animales/:id/hermanos`.<br>2. El sistema busca animales cuyo `padre` o `madre` sea el ID dado (hijos), o que compartan ambos padres (hermanos).<br>3. Responde `200 OK` con la lista. |
...
| **Flujo normal**| 1. Admin envía `GET /api/usuarios` o `PATCH /api/usuarios/:id` con body `{ active: false }`.<br>2. Verifica rol admin.<br>3. Ejecuta la operación (soft delete en usuarios también).<br>4. Responde `200 OK`. |
...
| POST     | `/api/especies`                       | Sí       | admin  | Crear especie                        |
| GET      | `/api/especies`                       | Sí       | —      | Listar especies                      |
| GET      | `/api/especies/:id`                   | Sí       | —      | Detalle de especie                   |
| PUT      | `/api/especies/:id`                   | Sí       | admin  | Actualizar especie                   |
| DELETE   | `/api/especies/:id`                   | Sí       | admin  | Soft delete especie                  |
| POST     | `/api/razas`                          | Sí       | admin  | Crear raza                           |
| GET      | `/api/razas`                          | Sí       | —      | Listar razas                         |
| GET      | `/api/razas/:id`                      | Sí       | —      | Detalle de raza                      |
| PUT      | `/api/razas/:id`                      | Sí       | admin  | Actualizar raza                      |
| DELETE   | `/api/razas/:id`                      | Sí       | admin  | Soft delete raza                     |
| POST     | `/api/animales`                       | Sí       | —      | Crear animal                         |
| GET      | `/api/animales`                       | Sí       | —      | Listar animales                      |
| GET      | `/api/animales/:id`                   | Sí       | —      | Detalle de animal                    |
| PUT      | `/api/animales/:id`                   | Sí       | —      | Actualizar animal                    |
| DELETE   | `/api/animales/:id`                   | Sí       | —      | Soft delete animal                   |
| PATCH    | `/api/animales/:id/padres`            | Sí       | —      | Asignar padres                       |
| GET      | `/api/animales/:id/arbol-genealogico` | Sí       | —      | Árbol genealógico                    |
| GET      | `/api/animales/:id/hijos`             | Sí       | —      | Hijos directos                       |
| GET      | `/api/animales/:id/hermanos`          | Sí       | —      | Hermanos                             |
| GET      | `/api/usuarios`                       | Sí       | admin  | Listar usuarios                      |
| PATCH    | `/api/usuarios/:id`                   | Sí       | admin  | Desactivar usuario (body: `{ active: false }`) |
